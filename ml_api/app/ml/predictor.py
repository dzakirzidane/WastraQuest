...
def load_models():
    ...
    global _rf_model, _svm_model, _scaler, _feature_columns
    ...
    for path in [rf_path, svm_path, scaler_path, features_path]:
...
def predict(jawaban_benar: int, tingkat_kesulitan: str) -> dict:
    ...
    if _rf_model is None or _svm_model is None or _scaler is None:
        load_models()

    X = _build_feature_vector(jawaban_benar, tingkat_kesulitan)
    X_scaled = _scaler.transform(X)

    rf_pred = _rf_model.predict(X_scaled)[0]
    rf_proba = _rf_model.predict_proba(X_scaled)[0][1]  # probabilitas kelas "Lulus"

    svm_pred = _svm_model.predict(X_scaled)[0]
    svm_proba = _svm_model.predict_proba(X_scaled)[0][1]

    return {
        "random_forest": {
            "prediksi": "Lulus" if rf_pred == 1 else "Tidak Lulus",
            "probabilitas_lulus": round(float(rf_proba), 4),
        },
        "svm": {
